...
# is maintained by hand -- by this script, rather than by editing eight URLs and
...
# Matching the timeouts the vendor's own installer uses. Without them a CDN that
# accepts the connection and then never answers hangs this script forever, with
# no output and nothing to interrupt but the terminal.
...
# The vendor's platform name on the left, mise's on the right. All four rather
# than just the macOS pair: `mise install` fails outright on a platform the pin
# has no URL for, and this repository's Python half is developed and CI-tested
# on Linux. The vendor also ships linux-*-musl builds, which are deliberately
# not pinned -- mise's platform key is `linux-x64` either way, so there is no
# way to express "musl" here, and Alpine is not a machine anyone builds this on.
PLATFORMS="darwin-arm64:macos-arm64 darwin-x64:macos-x64 linux-x64:linux-x64 linux-arm64:linux-arm64"

# Which build this machine can actually execute, and therefore the only one we
# can ask for a version number. The CDN serves one release across platforms, so
# that answer stands for all four pins.
case "$(uname -s)" in
  Darwin) host_os=darwin ;;
  Linux)  host_os=linux ;;
  *) echo "ERROR: run this on macOS or Linux" >&2; exit 1 ;;
esac
case "$(uname -m)" in
...
for pair in $PLATFORMS; do
  ...
  # is the failure this catches, and pinning a hash we never checked against its
  # own bytes would make the mise-side checksum decorative.
...
# Compare against what is pinned now, so a no-op run says so instead of touching
# the file and leaving an empty-looking diff to interpret.
...
# Replace the marked region in place. awk rather than sed -i: the replacement is
# multi-line and contains URLs full of characters sed would treat as delimiters
# or backreferences.
awk -v blockfile="$NEW" '
